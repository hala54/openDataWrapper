...
&lt;http://lodpaddle.univ-nantes.fr/dataDavy&gt; rdf:type void:Dataset ;
	foaf:homepage "http://data.paysdelaloire.fr/donnees/detail/hebergements-labellises-tourisme-et-handicap-en-loire-atlantique";
	dcterms:title "Hébergements labellisés "Tourisme et Handicap" en Loire-Atlantique"^^xsd:string ;
	dcterms:description "Hébergements labellisés "Tourisme et Handicap" en Loire-Atlantique"^^xsd:string ;
	dcterms:created "2013-10-06"^^xsd:date;
...
<xsl:text>&#009;rdf:type&#009; pdll:dataDavy;
</xsl:text>
...
<xsl:otherwise><xsl:value-of select="concat(concat('&lt;http://lodpaddle.univ-nantes.fr/dataDavy/',encode-for-uri(.)),'&gt;')"/>&#009; foaf:name &#009; "<xsl:value-of select="translate(., '&quot;','')"/>"^^xsd:string ;
</xsl:otherwise></xsl:choose></xsl:template>
