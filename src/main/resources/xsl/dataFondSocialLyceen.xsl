...
&lt;http://lodpaddle.univ-nantes.fr/dataFondSocialLyceen&gt; rdf:type void:Dataset ;
...
<xsl:text>&#009;rdf:type&#009; pdll:dataFondSocialLyceen;
</xsl:text>
...
<xsl:otherwise><xsl:value-of select="concat(concat('&lt;http://lodpaddle.univ-nantes.fr/dataFondSocialLyceen/',encode-for-uri(.)),'&gt;')"/>&#009; foaf:name &#009; "<xsl:value-of select="translate(., '&quot;','')"/>"^^xsd:string ;
...
<xsl:template match="MONTANT_VOTE"><xsl:choose><xsl:when test=". = 'null'">&#009;gr:hasValueInteger&#009; "-120404040"^^xsd:int ;
</xsl:when>
<xsl:otherwise>&#009;gr:hasValueInteger&#009; "<xsl:value-of select="."/>"^^xsd:int ;
</xsl:otherwise></xsl:choose></xsl:template>
