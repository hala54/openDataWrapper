...
&lt;http://lodpaddle.univ-nantes.fr/parcsRelais&gt; rdf:type void:Dataset ;
	foaf:homepage "http://data.loire-atlantique.fr/donnees/detail/parcs-relais-de-nantes-metropole/";
	dcterms:title "Localisation des parcs relais de Nantes Métropole"^^xsd:string ;
	dcterms:description "Localisation des parcs relais de Nantes Métropole"^^xsd:string ;
...
<xsl:text>&#009;rdf:type&#009; pdll:parcsRelais;
</xsl:text>
...
<xsl:otherwise><xsl:value-of select="concat(concat('&lt;http://lodpaddle.univ-nantes.fr/parcsRelais/',encode-for-uri(.)),'&gt;')"/>&#009; foaf:name &#009; "<xsl:value-of select="translate(., '&quot;','')"/>"^^xsd:string ;
</xsl:otherwise></xsl:choose></xsl:template>
...
<xsl:template match="CAPACITE_VEHICULE_ELECTRIQUE"><xsl:choose><xsl:when test=". = 'null'">&#009;pdll:electricCarCapacity&#009; "-120404040"^^xsd:int ;
</xsl:when>
<xsl:otherwise>&#009;pdll:electricCarCapacity&#009; "<xsl:value-of select="."/>"^^xsd:int ;
</xsl:otherwise></xsl:choose></xsl:template>
