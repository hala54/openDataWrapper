...
&lt;http://lodpaddle.univ-nantes.fr/dataLieuxCulte&gt; rdf:type void:Dataset ;
...
<xsl:text>&#009;rdf:type&#009; pdll:dataLieuxCulte;
</xsl:text>
...
<xsl:otherwise><xsl:value-of select="concat(concat('&lt;http://lodpaddle.univ-nantes.fr/dataLieuxCulte/',encode-for-uri(.)),'&gt;')"/>&#009; foaf:name &#009; "<xsl:value-of select="translate(., '&quot;','')"/>"^^xsd:string ;
</xsl:otherwise></xsl:choose></xsl:template>
