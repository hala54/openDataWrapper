...
public class LoadLinks {
	...
	/**
	 * Load data informations from the mapping file and construct the silk's
	 * config file
	 * 
	...
	public void Load() throws FileNotFoundException, IOException {
		...
		p.load(new FileReader("src/main/config/linkmapping.properties"));

		for (Object t : p.keySet()) {

			// Kitems[0]=srce1,
			// items[1]=comparedProp1,items[2]=src2,items[3]=comparedProp2,items[4]=tranfoType,items[5]=linkURI,items[6]=graph1,items[7]=graphe2
			String map = p.getProperty((String) t);
			if (!map.isEmpty()) {

				String[] items = map.split(",");
				XMLLinkConstructor constrArtCommunes = new XMLLinkConstructor(
						items[0], items[2], items[5], items[1], items[3],
						items[4]);
		...
		if (files != null) {
			for (File fi : files) {
				if (fi.isFile() == true && fi.getName().endsWith(".nt")) {
					...
					convertInTurtle(fi.getPath(), fi.getParent() + "/ttl/"
							+ fi.getName().split(".nt")[0] + ".ttl");
					i = i + 1;
	 ...
	 * @throws IOException
	 */
	public static void copy(String SourceFile, String NewDestFile)
			throws IOException {
	 ...
	 * @throws Exception
	 */
	public void deleteDuplicates(String srcFile, String linkFile, String output)
