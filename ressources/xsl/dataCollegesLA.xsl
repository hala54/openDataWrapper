...
&lt;http://lodpaddle.univ-nantes.fr/dataCollegesLA&gt; rdf:type void:Dataset ;
	foaf:homepage "http://data.nantes.fr/donnees/detail/colleges-prives-en-loire-atlantique";
	dcterms:title "Colleges prives en Loire-Atlantique"^^xsd:string ;
	dcterms:description "Colleges prives en Loire-Atlantique"^^xsd:string ;
...
<xsl:text>&#009;rdf:type&#009; pdll:dataCollegesLA;
</xsl:text>
...
<xsl:otherwise><xsl:value-of select="concat(concat('&lt;http://lodpaddle.univ-nantes.fr/dataCollegesLA/',encode-for-uri(.)),'&gt;')"/>&#009; foaf:name &#009; "<xsl:value-of select="translate(., '&quot;','')"/>"^^xsd:string ;
</xsl:otherwise></xsl:choose></xsl:template>
